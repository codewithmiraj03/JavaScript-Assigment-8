...
let minVar = products.map(product => (product.varations).reduce((priv, curr) => priv.price < curr.price? priv:curr));
...
let review = products.map(p => p.reviews.length)

console.log(review);

// 3- Count the average rating of each product.

let avgrat = products.map(avg =>{
  let avrge = avg.reviews.reduce((acc,curr) => (acc + curr.rating), 0);

  return  avrge / avg.reviews.length
...
let each = products.map(product => ({id: product.id,title: product.title, reviews: product.reviews.filter(review => !review.status)}));
